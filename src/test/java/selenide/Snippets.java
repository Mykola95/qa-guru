...
public class Snippets {
    ...
    void browser_command_examples() {
        ...
        Selenide.switchTo().defaultContent(); // вернутись назад до головної сторінки
    ...
    void selectors_examples() {
        ...
        $(byText("full text")).click(); // повне співпадіння по тексту
        $(withText("ull tex")).click();

        $("").parent(); // шукає батька
        $("").sibling(1); // вниз по сусідам
        $("").preceding(1); // вверх по сусідам
        $("").closest("div"); // вверх по дереву
        $("").ancestor("div"); // the same as closest
        $("div:last-child").lastChild();
    ...
    void collections_examples() {
        ...
        $$("div").excludeWith(text("123")).shouldHave(size(1)); // протилежне до методу вище
